...
def test_session():
    # Patch configs["database_url"] to use SQLite in-memory DB
    with patch.dict("app.configs.configs", {"database_url": "sqlite:///:memory:"}):
...
def test_get_user_by_email(test_session):
...
def test_update_user(test_session):
    ...
    created_user = User.create(test_session, email=user_data["email"], password=user_data["password"])
    updated_user = User.update(test_session, created_user["id"], **user_data["update_fields"])
...
def test_get_by_id_not_found(test_session):
    missing_id = TEST_DATA["not_found_tests"]["nonexistent_id"]
    with pytest.raises(RecordNotFoundError):
        User.get_by_id(test_session, missing_id)

def test_get_user_by_email_not_found(test_session):
    missing_email = TEST_DATA["not_found_tests"]["nonexistent_email"]
    with pytest.raises(ValueError):
        User.get_user_by_email(test_session, missing_email)

def test_update_json_field_primary_metadata(test_session):
    user_data = TEST_DATA["json_field_update"]
    created_user = User.create(test_session, email=user_data["email"], password=user_data["password"])
    json_update = user_data["json_update"]
    updated_user = User.update_json_field(
        test_session, created_user["id"],
        json_update["column"], json_update["key"], json_update["value"]
    )
    assert updated_user[json_update["column"]][json_update["key"]] == json_update["value"]

def test_update_json_field_invalid_column(test_session):
    user_data = TEST_DATA["invalid_json_field"]
    created_user = User.create(test_session, email=user_data["email"], password=user_data["password"])
    json_update = user_data["json_update"]
    # ✅ Fixed error message expectation:
    with pytest.raises(ValueError, match=f"Column {json_update['column']} does not exist on the model."):
        User.update_json_field(
            test_session, created_user["id"],
            json_update["column"], json_update["key"], json_update["value"]
        )

def test_update_json_field_record_not_found(test_session):
    missing_id = TEST_DATA["not_found_tests"]["nonexistent_id"]
    json_update = TEST_DATA["json_field_update"]["json_update"]
    with pytest.raises(RecordNotFoundError):
        User.update_json_field(
            test_session, missing_id,
            json_update["column"], json_update["key"], json_update["value"]
        )
