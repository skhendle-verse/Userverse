...
def test_a_create_company_one_roles_success(client, login_token, test_company_data):
    ...
    for role_key, role_value in roles.items():
        ...
        assert json_data["message"] == CompanyResponseMessages.ROLE_CREATION_SUCCESS.value
        assert "data" in json_data
        assert json_data["data"]["name"] == role_value["name"]
        assert json_data["data"]["description"] == role_value["description"]


def test_a_create_company_two_roles_success(client, login_token_user_two, test_company_data):
    """
    Test creating roles for a company successfully.
    """
    roles = test_company_data["roles"]
...
def test_b_create_company_roles_failure(client, login_token_user_two, test_company_data):
    """
...
def test_c_create_company_roles_failure(client, login_token_user_two, test_company_data):
    """
    ...
    for role_key, role_value in roles.items():
        ...
        #
        assert "detail" in json_data
        assert json_data["detail"]["message"] == CompanyResponseMessages.ROLE_CREATION_FAILED.value
