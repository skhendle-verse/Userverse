...
class TestDatabaseConfigWithLogging(unittest.TestCase):
    ...
    def test_test_environment_sqlite(self, mock_logger):
        config = {}
        result = DatabaseConfig.get_connection_string(config, environment="test_environment")
    ...
    def test_no_database_config(self, mock_logger):
        ...
        mock_logger.warning.assert_called_with("No database config found. Falling back to SQLite.")
    ...
    def test_missing_fields_fallback_to_sqlite(self, mock_logger):
        ...
        self.assertIn("Missing database config fields", mock_logger.warning.call_args[0][0])
    ...
    def test_unsupported_type_fallback(self, mock_logger):
        ...
        mock_logger.warning.assert_called_with("Unsupported database type: 'oracle'. Falling back to SQLite.")
    ...
    def test_postgresql_connection_string(self):
        ...
        result = DatabaseConfig.get_connection_string(config, environment="production")
        self.assertEqual(result, "postgresql+psycopg2://postgres:pass@localhost:5432/mydb")
